...
- iOS first, Android in stages: `platforms: ["ios", "android"]`. iOS is the full app (HIG, SwiftUI islands, iCloud backup). Android ships stage by stage per [ADR 0025](docs/adr/0025-android-staged-migration.md) — stage 1 (all plan weeks, clock + vibration cues, no location/speech/map/Health/elevation) is built; the ADR's stage table says what is live. The two apps are deliberately not a 1:1 design: Material 3 with Jetpack Compose islands on Android, dynamic (wallpaper) colour. There is no web target — react-native-web has been removed.
