...
- iOS first, Android in stages: `platforms: ["ios", "android"]`. iOS is the full app (HIG, SwiftUI islands, iCloud backup). Android ships stage by stage per [ADR 0025](docs/adr/0025-android-staged-migration.md) — stages 1 (all plan weeks, clock + vibration cues) and 2 (foreground-service location: distance, pace, screen-off heartbeat) are built; no speech/map/Health/elevation yet; the ADR's stage table says what is live. The two apps are deliberately not a 1:1 design: Material 3 with Jetpack Compose islands on Android, dynamic (wallpaper) colour. There is no web target — react-native-web has been removed.
...
The `/ios` and `/android` folders are gitignored — both are generated via prebuild (Continuous Native Generation). Never edit native projects directly; configure everything through `app.json` and config plugins.
