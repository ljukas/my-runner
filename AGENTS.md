...
- iOS first, Android in stages: `platforms: ["ios", "android"]`. iOS is the full app (HIG, SwiftUI islands, iCloud backup). Android ships stage by stage per [ADR 0025](docs/adr/0025-android-staged-migration.md) — stages 1 (all plan weeks, clock + vibration cues), 2 (foreground-service location: distance, pace, screen-off heartbeat), 3 (spoken cues over audio focus via the local `modules/audio-focus/` module), 4 (Google Maps route card and viewer; needs the Maps key in `.env.local`, see "Maps" below) and 5 (Health Connect via `react-native-health-connect` behind the `HealthAdapter` port, plus the local `modules/launch-intent/` module for its rationale intent) are built; no elevation yet; the ADR's stage table says what is live. The two apps are deliberately not a 1:1 design: Material 3 with Jetpack Compose islands on Android, dynamic (wallpaper) colour. There is no web target — react-native-web has been removed.
...
The `/ios` and `/android` folders are gitignored — both are generated via prebuild (Continuous Native Generation). Never edit native projects directly; configure third-party native code through `app.json` and config plugins, and put native code the app owns in a local Expo module under `modules/` (`modules/audio-focus/` and `modules/launch-intent/`, both Android-only — ADR 0025's stage-3 and stage-5 amendments; autolinking picks them up from that directory, and their TS wrappers are imported via the `@/modules/*` alias).
