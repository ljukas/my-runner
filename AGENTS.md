...
- **Argent cannot see this app's UI — use Maestro to find and tap elements.** Every product screen is an `@expo/ui` SwiftUI island (ADR 0005), and argent's element-discovery tools return an *empty* tree for all of them: `describe` gives a bare `ROOT AXGroup`, `native-describe-screen` gives `elements: []`, `native-find-views` matches nothing, and `debugger-component-tree` shows only the RN shell with useless `0.50,0.50` coordinates. Nothing is broken and `restart-app` will not fix it — the SwiftUI host simply does not surface to argent's a11y bridge. Maestro's `inspect_screen` reads the full hierarchy, which is exactly why ADR 0016's text-first selectors work. So: **discover and interact with Maestro** (`inspect_screen`, then inline `run` YAML), and keep argent for what still works on this app — `screenshot`, `launch-app`, `open-url`, `boot-device`, Metro/debugger tools, and profiling. Stop argent's servers first (`stop-all-simulator-servers`); see the Tool split below for why. Verifying against the **dev client** (`se.lukaslindqvist.runbro.dev`) is fine for interactive checks and needs no e2e build — but note `restart-app` drops you at the dev-client launcher, not the app; get back in with `open-url` on `runbrodev://expo-development-client/?url=http://<lan-ip>:8081`.
