...
| 2 — Location & background | Foreground-service location (expo-location's Android config), fixes into the engine, distance and pace, the run keeps its heartbeat with the screen off; keep-awake reverts to lock-only. Location primer onboarding step returns. | `location-tracker/adapter.android.ts`; `runHoldsScreenAwake`; `RunLocationBanner` stops hiding on `'unsupported'`. | Planned — [plan](../superpowers/plans/2026-09-21-android-stage-2-location-heartbeat.md) |
