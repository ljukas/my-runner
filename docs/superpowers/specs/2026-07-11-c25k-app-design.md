...
| Run lock | Per-run lock on the run screen, **default unlocked, never persisted** (every run, including a resumed one, starts unlocked): locked holds the display awake *and* makes End/Pause/Skip inert, so a pocketed screen-on phone is safe from a stray tap. Tap to lock, press and hold 1.2 s to unlock. Its purpose is the foreground **haptic** cue accent channel, which iOS grants only to the foreground-active app — screen-on is the only switch that channel has (revised 2026-07-27, replacing a persisted screen-awake toggle; ADR 0009) |
...
| `expo-keep-awake` | Screen stays on while the run screen's lock is on | UX only, not a background strategy |
