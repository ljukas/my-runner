...
- iOS first, Android in stages: `platforms: ["ios", "android"]`. iOS is the full app (HIG, SwiftUI islands, iCloud backup). Android ships stage by stage per [ADR 0025](docs/adr/0025-android-staged-migration.md) — stages 1 (all plan weeks, clock + vibration cues), 2 (foreground-service location: distance, pace, screen-off heartbeat) and 3 (spoken cues over audio focus via the local `modules/audio-focus/` module) are built; no map/Health/elevation yet; the ADR's stage table says what is live. The two apps are deliberately not a 1:1 design: Material 3 with Jetpack Compose islands on Android, dynamic (wallpaper) colour. There is no web target — react-native-web has been removed.
